...
void Shader::transmitUniformData(gs2019::Matrix4x4 *modelMatrix, Material *material)
{
    if (!mCurrentCamera)
    {
        qDebug() << "ERROR: No camera set in shader!!!";
    }

    //glUniformMatrix4fv( vMatrixUniform, 1, GL_TRUE, mCurrentCamera->mVMatrix.constData());
    //glUniformMatrix4fv( pMatrixUniform, 1, GL_TRUE, mCurrentCamera->mPMatrix.constData());
    //glUniformMatrix4fv( mMatrixUniform, 1, GL_TRUE, modelMatrix->constData());

    glUniformMatrix4fv( vMatrixUniform, 1, GL_TRUE, mCurrentCamera->mVMatrix.transposed().constData());
    glUniformMatrix4fv( pMatrixUniform, 1, GL_TRUE, mCurrentCamera->mPMatrix.transposed().constData());
    glUniformMatrix4fv( mMatrixUniform, 1, GL_TRUE, modelMatrix->constData());
}
